...
class MovieDetailsViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        moviePoster.layer.cornerRadius = 2.0
        moviePoster.layer.borderWidth = 1.0
        moviePoster.layer.borderColor = UIColor.clear.cgColor
        moviePoster.layer.masksToBounds = true

        moviePoster.layer.shadowColor = UIColor.black.cgColor
        moviePoster.layer.shadowOffset = CGSize(width: 0, height: 2.0)
        moviePoster.layer.shadowRadius = 2.0
        moviePoster.layer.shadowOpacity = 0.5
        moviePoster.layer.masksToBounds = false
        moviePoster.layer.shadowPath = UIBezierPath(roundedRect: moviePoster.bounds, cornerRadius: moviePoster.layer.cornerRadius).cgPath
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.section == 0 {
            if let tralier = self.trailers?[indexPath.row] {
                self.openTrailer(trailer: tralier)
            } else {
    ...
    func openTrailer(trailer: Trailer) {
    ...
    func displayReview(index: Int) {
        if let review = self.reviews?[index]{
